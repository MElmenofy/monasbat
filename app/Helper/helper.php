...
// test status
if ($old_status == 'Pending' && $status == 'Accept'){
            $data['activity_message'] = __('messages.booking_status_update'). __('messages.from'). 'قيد الانتظار'. __('messages.to').'قبول';
        }elseif ($old_status == 'Accept' && $status == 'On Going'){
            $data['activity_message'] = __('messages.booking_status_update'). __('messages.from'). 'قبول'. __('messages.to').'جاري التنفيذ';
        }elseif ($old_status == 'On Going' && $status == 'Completed'){
            $data['activity_message'] = __('messages.booking_status_update'). __('messages.from'). 'جاري التنفيذ'. __('messages.to').'مكتمل';
        }
// /test status
            ...
            // $data['activity_message'] = __('messages.booking_status_update'). __('messages.from').$old_status. __('messages.to').$status;
            $activity_data = [
...
//            $data['activity_message'] = __('messages.booking_assigned',['name' => $assigned_handyman]);
            $data['activity_message'] = __('messages.booking_assigned'). __('messages.name'). $assigned_handyman;
//            $data['activity_message'] = __('messages.booking_assigned');
            $activity_data = [
...
//            $data['activity_message'] = __('messages.booking_transfer',['name' => $assigned_handyman]);
            $data['activity_message'] = __('messages.booking_transfer'). __('messages.name'). $assigned_handyman;
//            $data['activity_message'] = __('messages.booking_transfer');
            $activity_data = [
                'handyman_id' => $data['booking']->handymanAdded->pluck('handyman_id'),
                'handyman_name' => $data['booking']->handymanAdded,
            ];
...
//            $data['activity_message'] = __('messages.payment_message',['status' => $data['payment_status'] ]);
            // $data['activity_message'] = __('messages.payment_message'. $data['payment_status']);
//            $data['activity_message'] = __('messages.payment_message');

            if ($data['payment_status'] == 'paid'){
                $data['activity_message'] = __('messages.payment_message'). ' الي مدفوع ';
            }else{
                $data['activity_message'] = __('messages.payment_message');
            }
            $activity_data = [
...
function getPriceFormat($price){
    ...
    if ($position == 'left') {
        $price = $symbol."".number_format($price);
    } else {
